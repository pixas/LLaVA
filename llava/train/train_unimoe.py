...
def safe_save_model_for_hf_trainer(trainer: transformers.Trainer,
    ...
    if getattr(trainer.args, "tune_mm_mlp_adapter", False):
        ...
        #     other_weights = (get_peft_state_non_lora_maybe_zero_3(trainer.model.named_parameters()))
        ...
        if trainer.args.local_rank == 0 or trainer.args.local_rank == -1:
            if current_folder.startswith('checkpoint-'):
                ...
                # if getattr(trainer.args, "tune_entire_model", False):
                #     torch.save(other_weights, os.path.join(output_dir, f'{current_folder}.bin'))
            else:
                torch.save(weight_to_save, os.path.join(output_dir, f'mm_projector.bin'))
                # if save_vision:
                #     torch.save(vision_tower_weight, os.path.join(output_dir, f'vision_tower.bin'))
                # if getattr(trainer.args, "tune_entire_model", False):
                #     torch.save(other_weights, os.path.join(output_dir, f'other_weights.bin'))
        if getattr(trainer.args, "tune_vision_tower", False):
            ...
            if trainer.args.local_rank == 0 or trainer.args.local_rank == -1:
                if current_folder.startswith('checkpoint-'):
                    ...
                    torch.save(weight_to_save, os.path.join(
                        vision_folder, f'{current_folder}.bin'))
                else:
                    torch.save(weight_to_save, os.path.join(output_dir, 'vision_tower.bin'))
