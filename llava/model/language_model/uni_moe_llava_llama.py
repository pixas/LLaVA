...
class UniMoELlavaConfig(UniMoELlamaConfig):
    model_type = "unimoe_llava"
    


class UniMoELLamaMLP(nn.Module):
    def __init__(self, config) -> None:
        super(UniMoELLamaMLP, self).__init__()
...
class UniMoELlamaModel(LlamaModel):
    def __init__(self, config: UniMoELlamaConfig):
        ...
        if config.moe_layer_index == -1:
            ...
            self.layers = nn.ModuleList([UniMoELLamaDecoderLayer(config) if i < moe_layer_index else LlamaDecoderLayer(config) for i in range(config.num_hidden_layers)])
        self.post_init()
    
    def forward(self, input_ids: torch.LongTensor = None, attention_mask: torch.Tensor | None = None, position_ids: torch.LongTensor | None = None, past_key_values: List[torch.FloatTensor] | None = None, inputs_embeds: torch.FloatTensor | None = None, use_cache: bool | None = None, output_attentions: bool | None = None, output_hidden_states: bool | None = None, return_dict: bool | None = None) -> Tuple | UniMoEBaseModelOutputWithPast:
        ...
        return UniMoEBaseModelOutputWithPast(
...
class UniMoELlavaLlamaModel(MoELlavaMetaModel, UniMoELlamaModel):
    config_class = UniMoELlavaConfig
    
    def __init__(self, config: UniMoELlamaConfig):
        super(UniMoELlavaLlamaModel, self).__init__(config)

class UniMoELlamaForCausalLM(LlamaForCausalLM):
    config_class = UniMoELlamaConfig
    def __init__(self, config):
        super().__init__(config)
        self.model = UniMoELlamaModel(config)
        # self.lm_head = nn.Linear(config.hidden_size, config.vocab_size, bias=False)
        # self.load_balancing_loss_ceof = 0.01
        # Initialize weights and apply final processing
        self.post_init()

class UniMoELlavaLlamaForCausalLM(UniMoELlamaForCausalLM, MoELlavaMetaForCausalLM):
    config_class = UniMoELlavaConfig
    
    def __init__(self, config):
        super(UniMoELlavaLlamaForCausalLM, self).__init__(config)
        self.model = UniMoELlavaLlamaModel(config)
        self.lm_head = nn.Linear(config.hidden_size, config.vocab_size, bias=False)
        self.load_balancing_loss_ceof = 0.01
        # Initialize weights and apply final processing
        self.post_init()
...
AutoConfig.register("unimoe_llama", UniMoELlamaConfig)
AutoConfig.register("unimoe_llava", UniMoELlavaConfig)
AutoModelForCausalLM.register(UniMoELlamaConfig, UniMoELlamaForCausalLM)
AutoModelForCausalLM.register(UniMoELlavaConfig, UniMoELlavaLlamaForCausalLM)
