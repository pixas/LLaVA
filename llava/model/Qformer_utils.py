...
class MoEBertLayer(BertLayer):
    ...
    def forward(
        ...
        if query_length > 0:
            ...
            moe_output = apply_chunking_to_forward(
                self.feed_forward_chunk_query,
                self.chunk_size_feed_forward,
                self.seq_len_dim,
                query_attention_output,
...
class BertEncoder(nn.Module):
    ...
    def forward(
        ...
        return {
            ...
            "moe_output": moe_output if len(moe_output[0]) > 0 else None
        }
...
class BertLMHeadModel(BertPreTrainedModel):
    ...
    def forward(
        ...
        outputs = temp_outputs['bert_output']
        if temp_outputs['moe_output'] is not None:
            moe_output = temp_outputs['moe_output']
        else:
            moe_output = None
        ...
        return {
            "bert_output": CausalLMOutputWithCrossAttentions(
                loss=lm_loss,
